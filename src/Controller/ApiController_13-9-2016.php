...
class ApiController extends AppController
{
     ...
     public function initialize()
    {
	 ...
	 public function beforeFilter(Event $event) {
    	...
    	$this->Auth->allow(['getUserScore','getrank','getAllUser','getscorerange','addChessman','getuserseacrh','editimage','getuser','getranking','getsingleitem','getitems','getscore','itemSearch','redemptionRecords','saveUserScore','getAllInfoCraftman','updateScore','getgamerule','login','getquestions','getcorrectanswer','register','userInformation','getAllGames','getAllItems']);
...
$image="";
//$image=(isset($game[0]['image']) && !empty($game[0]['image'])) ? HTTP_ROOT.'img/uploads/'.$game[0]['image'] : '';
}
if($value['cat_id']==10){
...
$usergameplayedquery = $this->UserGamePlayed->find('all')
			...
			->where(['UserGamePlayed.user_id'=>$_GET['user_id'],'UserGamePlayed.game_id'=>$value['id']])
													->hydrate(false)
													  ->toArray();
													 // $updateall=$usergameplayedquery;
												//	  print_r($usergameplayedquery);die;
									//$total_scoredata=$usergameplayedquery[0]['score']+$scorerequest;
									if($usergameplayedquery){
												$updateall= '1';
