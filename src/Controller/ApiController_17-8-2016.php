...
class ApiController extends AppController
{
     ...
     public function initialize()
    {
	 ...
	 public function beforeFilter(Event $event) {
    	...
    	$this->Auth->allow(['addChessman','getuserseacrh','editimage','getuser','getranking','getsingleitem','getitems','getscore','itemSearch','redemptionRecords','saveUserScore','getAllInfoCraftman','updateScore','getgamerule','login','getquestions','getcorrectanswer','register','userInformation','getAllGames','getAllItems']);
            ...
            $auth = $this->Api->isExistBytoken($this->request->data['login_token']);
              if($auth == true){
                  $data=$this->request->data;
                      ...
                      if($query){
                      $score = $this->UsersScore->find('all')
                                      ->select(['total'=>'SUM(UsersScore.score + UsersScore.bonus)'])
                                      ->where(['UsersScore.user_id'=>$user_id])
                        ...
                        $query = $this->Items->find('all')->where(['Items.score <='=> $total_score])->hydrate(false)->toArray();
   ...
   /**
    ...
    public function getAllItems(){
      ...
      if($this->request->is('GET')){
        ...
        if($auth == true){
          $this->loadModel('Items');
          $items = $this->Items->find('all')->select(['id','item_title','image','score'])->order(['Items.id'=>'DESC'])->hydrate(false)->toArray();
          if($items){
            foreach ($items as $key => $value) {
