...
class ApiController extends AppController
{
     ...
     public function initialize()
    {
	 ...
	 public function beforeFilter(Event $event) {
    	...
    	$this->Auth->allow(['adminUserScore','getUserScore','getrank','getAllUser','getscorerange','addChessman','getuserseacrh','editimage','getuser','getranking','getsingleitem','getitems','getscore','itemSearch','redemptionRecords','saveUserScore','getAllInfoCraftman','updateScore','getgamerule','login','getquestions','getcorrectanswer','register','userInformation','getAllGames','getAllItems']);
...
$query_status=$this->Users->updateAll(array('login_token'=>uniqid()),array('email'=>$_POST['email']));
                            $error_DB=$this->Users->find('all')->select(['id','name','email','contact_no','login_token','device_id','profile_pic','department_id','role_id'])->where(array('email'=>$_POST['email']))->toArray();
...
if($score==1)
{
 ...
 //$userscoredata=$this->UsersScore->find('all')->where(['UsersScore.user_id'=>$this->request->data['user_id'] ])->hydrate(false)->toArray();
  $data = $this->Items->find('all')->where(['Items.item_title LIKE'=> '%'.$this->request->data['search'].'%', 'Items.score <='=>$score['total']])->hydrate(false)->toArray();
  if($data )
{
$arr = array();
   ...
   public function getAllGames(){
      ...
      if($this->request->is('GET')){
        ...
        if($auth == true){
          ...
          $this->loadModel('UserGamePlayed');
 $res = array();
        $game = $this->Game->find('all')->contain(['Category'])->order(['Category.id'=>'ASC'])->order(['Game.id'=>'DESC'])->hydrate(false)->toArray();
...
$userscoredata= $this->UserGamePlayed->find('all')
          ...
          $items = $this->Items->find('all')->select(['id','item_title','image','score'])->where(['Items.score <='=>$userscoredata['total']])->order(['Items.id'=>'DESC'])->hydrate(false)->toArray();
}
          if($items){
            foreach ($items as $key => $value) {
...
/*$userscoredata= $this->UserGamePlayed->find('all')->select(['UserGamePlayed.user_id','UserGamePlayed.game_id','UserGamePlayed.score','UserGamePlayed.created'])
                                      ...
                                      ->first()
                                      ->toArray();*/
$userscoredata= $this->UserGamePlayed->find('all')->where(['UserGamePlayed.user_id'=>$_GET['user_id']])->hydrate(false)->toArray();
