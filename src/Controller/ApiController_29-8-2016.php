...
class ApiController extends AppController
{
     ...
     public function initialize()
    {
	 ...
	 public function beforeFilter(Event $event) {
    	...
    	$this->Auth->allow(['getscorerange','addChessman','getuserseacrh','editimage','getuser','getranking','getsingleitem','getitems','getscore','itemSearch','redemptionRecords','saveUserScore','getAllInfoCraftman','updateScore','getgamerule','login','getquestions','getcorrectanswer','register','userInformation','getAllGames','getAllItems']);
...
$score_DB=$this->UsersScore->find('all')->where(['UsersScore.user_id'=>$error_DB[0]['id']])->toArray();
                              $error["statuscode"]= 200;
                             // $error["msg"]= "User has been login successfully.";
 $error["msg"]= "用戶已成功登入";
                              $error['data'] = $error_DB[0];
 $error['data']['score'] = $score_DB[0]['score'];
                              $response = $error;
                    }
                    else{
                    $response = $error;
...
public function getscore(){
		...
		if($usergameplayedquery){
			//echo $usergameplayedquery['score'];
       ...
       // $score->raneid = $game_id;
        ...
        $UserGamePlayedinsert = $this->UserGamePlayed->save($UserGamePlayeddata);
                      ...
                      $score = $this->UserGamePlayed->find('all')
                                    ...
                                    // print_r($score);die;
//print_r($score);
 //$total_scoredata=$score['score']+$scorerequest;
 //$updateall= $this->UsersScore->updateAll(array('score'=>$total_scoredata),array('game_id'=>$this->request->data['game_id'],'user_id'=>$this->request->data['user_id']));                                         
                      //  $total_score=$score['total'];
//$total_score=$total_scoredata+$score['bonus'];
