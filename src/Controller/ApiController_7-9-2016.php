        ...
        if($this->request->is('POST')){
            $auth = $this->Api->isExistBytoken($this->request->data['login_token']);
              if($auth == true){
				  ...
				  {
					  $craftman=array();
					 $craftman=$data['craftman'];
					 $datacraft=count($craftman);
					 $user_id= $craftman[0]['user_id'];
					 $game_id=$craftman[0]['game_id'];
                   	
					$scorerequest=$craftman[0]['score'];
					$score_id=$craftman[0]['score_id'];
