...
$this->loadModel('UsersScore');
 $query = $this->UsersScore->deleteAll(array('game_id'=>$this->request->data['id']));
     $this->saveIdentityScore($this->request->data['id'] ,$this->request->data['onsite_score'],$this->request->data['identity']);
                        if (!empty($this->request->data['image']['name'])) {
        ...
        if($this->request->is('POST')){
            ...
            $dateCheck = $this->dateCompare($startdatetime,$enddatetime);
                ...
                if($validateImage['error'] == 'false'){
                    $this->loadModel('Game');
                    $game = $this->Game->newEntity();
                    $this->request->data['startdate'] = $startdatetime;
                    $this->request->data['enddate'] = $enddatetime;
