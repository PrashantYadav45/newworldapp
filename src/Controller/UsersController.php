...
date_default_timezone_set('Asia/Hong_Kong');
    ...
    public function index() {
        ...
        $query = $this->Users->find('search', $this->Users->filterParams($this->request->query))->contain(['Departments','Role']);
  //echo"<pre>";print_r($query);die;
       /*  $new_items=$this->Departments->find('all')->order(['id'=>'desc'])->hydrate(false)->toArray();
    ...
    public function add() {
         ...
         $new_items=$this->Departments->find('all')->order(['id'=>'desc'])->hydrate(false)->toArray();
         $this->set(compact('new_items')); 
         $Role=$this->Role->find('all',array('conditions'=>array('Role.id >' => 1)))->order(['id'=>'desc'])->hydrate(false)->toArray();
 ...
 $post_data['createdAt']=date("Y-m-d H:i:s");
          //  $post_data['createdAt']=Time::parseDateTime(date("Y-m-d H:i:s"), 'yyyy/MM/dd HH:mm:ss');
            $user->department_id = $post_data['department'];
            unset($post_data['department']);
   	    $user->role_id = $post_data['role_id'];
	...
	//user change status//
  	 public function roles(){
            $status_msg=array('2'=>'sub_admin','3'=>'user');
            $response=array();
             if($this->request->is('get')){

                    $edit_id = $this->request->query['id'];
                    $status = $this->request->query['role_id'];
                    ...
                    $status_arr = array("role_id"=>3);

                    $this->Users->updateAll( $status_arr,  array('id' => $edit_id ));


                    }elseif($status=="3"){
                    $status_arr = array("role_id"=>2);
                    ...
                    $this->Users->updateAll( $status_arr,  array('id' => $edit_id ));
                        ...
                        $response['error']='false';
                        $response['result']=$result;
                        $response['role']=$status_arr['role_id'];
    ...
    public function forgotPass(){
        ...
        if($this->request->is('post')){
            $post_data = $this->request->data;
            $email = $this->Users->find('all')->where(['staffid=' => $post_data['email'],'role_id'=>1])->toArray();
            
            if(count($email)){
                $id=$email[0]->id;
                $name=$email[0]->name;
  $Staffid=$email[0]->staffid;
                ...
                $pass_vars = array('code' => $code,'name'=>$name,'Staffid'=>$Staffid,'logo'=>'http://localhost/newworld/img/small.png');
