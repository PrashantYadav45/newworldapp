...
<?php echo $this->Html->script('ckeditor.js');?>
 ...
 <?= $this->Flash->render();?>
                    <h1 class="page-header"><i class="fa fa-user"></i> 編輯 i-world</h1>
            ...
            <div class="row spacing">
                 <div class="col-lg-12">
                   ...
                   <form onsubmit="return atleast_onecheckbox(event)" action="<?php echo HTTP_ROOT .'iworld/edit/'.$data[0]['id']?>" enctype='multipart/form-data' method="post" id="sign-up_area" role="form">
...
<!-- Tabloid Game div --> 
	...
	<?php }?>	  
                      ...
                      <div id='tab<?=$j?>'>
                        <div class="form-group">
                          <label for="comment">題:</label>
                          <textarea name="question_text[<?=$j?>][]" class="form-control" rows="4" id='queationtag<?=$j?>' required><?= $data[$j]['question']['question_text'];?></textarea>
                        </div>
...
<input type="button" id='delete-tab' onclick="deletequestion()" value="刪除問題"> 
                  <input type="button" id='add-tab' value="添加問題"> 
